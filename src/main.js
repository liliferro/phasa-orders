...
async function downloadOrder(format, selected) {
  ...
  const setStatus = (text) => {
    document
      .querySelectorAll("#exportStatus, #documentExportStatus")
      .forEach((box) => (box.textContent = text));
  };
  try {
    document
